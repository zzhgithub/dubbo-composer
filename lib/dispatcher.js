...
Dispatcher.prototype.config = function (config) {
  // 初始化时的链接实例个数
  this._init_conn = config.initConn ? this._init_conn : config.initConn;
  // 使用时最大弹性链接数
  this._max_conn = config.maxConn ? this._max_conn : config.maxConn;
  // 使用时最小链接数
  this._min_conn = config.minConn ? this._min_conn : config.minConn;
